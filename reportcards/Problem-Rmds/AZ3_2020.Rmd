...
filenames = list.files(here("CD_Dirs/AZ3"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/AZ3", i)))
}

activefacilities <- `active-facilities_All_pg3_AZ-3`
...
# AZ3 <- housemembers[which(housemembers$CD=="AZ3"), ]
...
##### Arizona's 3rd District; seat held by `r leg_info$name` since `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/AZ3_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_AZ-3`
...
enforcement <- `enforcements_All_pg3_AZ-3`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="AZ3", "AZ3", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="AZ3", "AZ3", "Other")
...
AZ3_dot <- CWAper1000[which(CWAper1000$CD.State=="AZ3"), ]
...
ggsave("dotplot_AZ3.png", dotplot, path=here("CD_Dirs/AZ3"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/AZ3/dotplot_AZ3.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In AZ3, there were <span style="color:#569b5e">`r AZ3_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for AZ 3
CAArecurring <- `recurring-violations_All_pg3_AZ-3`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("AZ 3")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("AZ 3")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_AZ-3`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_AZ-3`
...
```
# Highlights for Arizona's 3rd District
...
names(`inspectionsper1000_All_pg4_AZ-3`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_AZ-3`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_AZ-102320.csv`, `inspectionsper1000_All_pg4_AZ-3`)
...
names(`violationsper1000_All_pg4_AZ-3`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_AZ-3`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_AZ-102320.csv`, `violationsper1000_All_pg4_AZ-3`)
...
CAAbadactors <- `noncomp_CAA_pg6_AZ-3`
...
#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_AZ-3`
...
#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_AZ-3`
...
ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/AZ3"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/AZ3"), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/AZ3"), scale = 3, width=7, height=4, units=c("cm"))

```
<a ><img src="../CD_Dirs/AZ3/CAAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/AZ3/CWAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
<a ><img src="../CD_Dirs/AZ3/RCRAfacilities.png" style="float: left; width: 65%; height: 60%; margin-right: 5%" ></a>
...
#clean up data
CAAviolations <- `violations_CAA_pg5_AZ-3` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_AZ-3`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_AZ-3`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_AZ-3`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_AZ-3`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_AZ-3`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_AZ-3`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_AZ-3`
...
<br />
<a ><img src="../CD_images/AZ3_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/AZ3_2020.pdf)
