...
data_set_list = c('RCRA Violations', 'RCRA Inspections', 'RCRA Penalties', 'CAA Violations', 'CAA Inspections', 'CAA Penalties', 'CWA Violations', 'CWA Inspections', 'CWA Penalties')
region$create_data_sets( data_set_list )
```


```{r readData, echo=FALSE}
conn <- dbConnect( RSQLite::SQLite(), "region.db")
per_1000 = dbGetQuery( conn, "select p1.program as program, p1.type as type,
    p1.count as count from per_1000 p1, regions r
    where r.region_type='USA' and r.rowid=p1.region_id")
national_info <- list( program=per_1000$program, type=per_1000$type, count=per_1000$count )
```
...
filenames = list.files(here("CD_Dirs/TX34"), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*-[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here("CD_Dirs/TX34", i)))
}

activefacilities <- `active-facilities_All_pg3_TX-34`
...
##### Texas' 34th District; seat held by `r leg_info$name` since `r leg_info$since_year` 
 <div class="container">
  <img src="../CD_maps/TX34_map.png" alt="Snow" style="width:100%;">
...
CWAviolations <- `violations_CWA_pg6_TX-34`
...
enforcement <- `enforcements_All_pg3_TX-34`
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State=="TX34", "TX34", "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State=="TX34", "TX34", "Other")
...
TX34_dot <- CWAper1000[which(CWAper1000$CD.State=="TX34"), ]
...
ggsave("dotplot_TX34.png", dotplot, path=here("CD_Dirs/TX34"), scale = 2, width=8, height=5, units=c("cm"))

```
 
<a ><img src="../CD_Dirs/TX34/dotplot_TX34.png" style="float: left; width: 100%; height: 60%; margin-right: 5%" ></a>
...
<h3>In TX34, there were <span style="color:#569b5e">`r TX34_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
#ok so We'll put the % in the title for the state, and try to figure out the same for TX 34
CAArecurring <- `recurring-violations_All_pg3_TX-34`[-c(2,3,5,6),]
...
CAArecurring[2,1] <- as.character("TX 34")
CAArecurring$Region2 <- as.character("All of OR")
CAArecurring[2,4] <- as.character("TX 34")
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- `violations_CWA_pg3_TX-34`
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- `inspections_All_pg3_TX-34`
...
```
# Highlights for Texas' 34th District
...
names(`inspectionsper1000_All_pg4_TX-112920.csv`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX-112920.csv`$Region <- as.factor("State")
names(`inspectionsper1000_All_pg4_TX-34`)[2] <- "Inspectionsper1000"
`inspectionsper1000_All_pg4_TX-34`$Region <- as.factor("Congressional District")
inspectionsper1000 <- rbind(USAinspectionsper1000_All, `inspectionsper1000_All_pg4_TX-112920.csv`, `inspectionsper1000_All_pg4_TX-34`)
...
names(`violationsper1000_All_pg4_TX-112920.csv`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX-112920.csv`$Region <- as.factor("State")
names(`violationsper1000_All_pg4_TX-34`)[2] <- "violationsper1000"
`violationsper1000_All_pg4_TX-34`$Region <- as.factor("Congressional District")
violationsper1000 <- rbind(USAviolationsper1000_All, `violationsper1000_All_pg4_TX-112920.csv`, `violationsper1000_All_pg4_TX-34`)
...
CAAbadactors <- `noncomp_CAA_pg6_TX-34`
...
if ( nrows_caa > 0 ) {
  ...
  ggsave("CAAfacilities.png", CAAfacilities, path=here("CD_Dirs/TX34"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
CWAbadactors <- `noncomp_CWA_pg6_TX-34`
...
if ( nrows_cwa > 0 ) {
  ...
  ggsave("CWAfacilities.png", CWAfacilities, path=here("CD_Dirs/TX34"), scale = 3, width=7, height=4, units=c("cm"))
}

#call in data and clean up 
RCRAbadactors <- `noncomp_RCRA_pg7_TX-34`
...
if ( nrows_rcra > 0 ) {
  ...
  ggsave("RCRAfacilities.png", RCRAfacilities, path=here("CD_Dirs/TX34"), scale = 3, width=7, height=4, units=c("cm"))
}
...
`r if ( nrows_caa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX34/CAAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**ECHO reports for facilities:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_rcra == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX34/RCRAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**ECHO reports for facilities:**</font><font size=\"1.5\"><br />"
  }`
...
`r if ( nrows_cwa == 0) { 
    ...
    "<br /><a ><img src=\"../CD_Dirs/TX34/CWAfacilities.png\" style=\"float: left; width: 55%; 
      height: 60%; margin-right: 5%\" ></a>
    <font size=\"3\">**ECHO reports for facilities:**</font><font size=\"1.5\"><br />"
  }`
...
#clean up data
CAAviolations <- `violations_CAA_pg5_TX-34` 
...
#clean up data
CAAinspections <- `inspections_CAA_pg5_TX-34`
...
#clean up data
CAAenforcement <- `enforcements_CAA_pg5_TX-34`
...
#clean up data
CWAinspections <- `inspections_CWA_pg6_TX-34`
...
#clean up data
CWAenforcement <- `enforcements_CWA_pg6_TX-34`
...
#clean up data
RCRAviolations <- `violations_RCRA_pg7_TX-34`
...
#clean up data
RCRAinspections <- `inspections_RCRA_pg7_TX-34`
...
#clean up data
RCRAenforcement <- `enforcements_RCRA_pg7_TX-34`
...
<br />
<a ><img src="../CD_images/TX34_rep.jpeg" style="float: left; width: 25%; margin-right: 5%" ></a>
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX34_2020.pdf)
