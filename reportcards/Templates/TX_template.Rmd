...
filenames = list.files(here(paste0("CD_Dirs/",cd_state)), pattern = ".csv")
file_count = length(filenames)
for (i in filenames){  
  name <- gsub("^([^_]*_[^_]*_[^_]*_[^-]*)-.*$","\\1", i)
  assign(name, read.csv(here(paste0("CD_Dirs/",cd_state), i)))
}

activefacilities <- get(paste0('active-facilities_All_pg3_', cd_state))
...
#map
map <- paste0("../../State_maps/", cd_state, "_map.png")
...
##### Texas; seats held by `r sen1_name` since `r sen1_since` and `r sen2_name` since `r sen2_since`
 <div class="container">
  `r map_tag `
  </div>
...
CWAviolations <- get(paste0('violations_CWA_pg6_',cd_state))
...
enforcement <- get(paste0('enforcements_All_pg3_',cd_state))
...
CWAper10002$Highlight <- ifelse(CWAper10002$CD.State==cd_state, paste0(cd_state), "Other")
CWAper1000$Highlight <- ifelse(CWAper1000$CD.State==cd_state, paste0(cd_state), "Other")
...
ggsave("dotplot.png", dotplot, path=here(paste0("CD_Dirs/", cd_state)), scale = 2, width=8, height=5, units=c("cm"))
...
dotplot <- paste0("../../CD_Dirs/", cd_state, "/dotplot.png")
...
<h3>In `r paste0(cd_state)` there were <span style="color:#569b5e">`r cd_state_dot[1,5]`</span> Clean Water Act violations per 100 regulated facilities in 2019.</h3>
...
```{r echo=FALSE}
#CAA
#megan 6/18/21 - I don't think we're actually using any of this data right now, so i'm going to block it out
##first let's fix up the data
#ok so We'll put the % in the title for the state, and try to figure out the same for the cd_state
...
#[9:11,2] Obama
# [17:19,2] Trump
effluentviolations <- get(paste0('violations_CWA_pg3_', cd_state))
...
#[9:11,2] Obama
# [17:19,2] Trump
inspections <- get(paste0('inspections_All_pg3_', cd_state))
...
inspections_cd_state <- get(paste0('inspectionsper1000_All_pg4_', cd_state))
...
violations_cd_state <- get(paste0('violationsper1000_All_pg4_', cd_state))
...
CAAbadactors <- get(paste0('noncomp_CAA_pg6_', cd_state))
...
#call in data and clean up 
CWAbadactors <- get(paste0('noncomp_CWA_pg6_', cd_state))
...
#call in data and clean up 
RCRAbadactors <- get(paste0('noncomp_RCRA_pg7_', cd_state))
...
ggsave("CAAfacilities.png", CAAfacilities, path=here(paste0("CD_Dirs/", cd_state)), scale = 3, width=7, height=4, units=c("cm"))
ggsave("CWAfacilities.png", CWAfacilities, path=here(paste0("CD_Dirs/", cd_state)), scale = 3, width=7, height=4, units=c("cm"))
ggsave("RCRAfacilities.png", RCRAfacilities, path=here(paste0("CD_Dirs/", cd_state)), scale = 3, width=7, height=4, units=c("cm"))

#make html tags for these images 
CAAfacilities <- paste0("../../CD_Dirs/", cd_state, "/CAAfacilities.png")
CAA_tag <- tags$img(src = CAAfacilities,style="float: left; width: 65%; height: 60%; margin-right: 5%")

CWAfacilities <- paste0("../../CD_Dirs/", cd_state, "/CWAfacilities.png")
CWA_tag <- tags$img(src = CWAfacilities,style="float: left; width: 65%; height: 60%; margin-right: 5%")


RCRAfacilities <- paste0("../../CD_Dirs/", cd_state, "/RCRAfacilities.png")
...
#clean up data
CAAviolations <- get(paste0('violations_CAA_pg5_', cd_state)) 
...
#clean up data
CAAinspections <- get(paste0('inspections_CAA_pg5_', cd_state))
...
#clean up data
CAAenforcement <- get(paste0('enforcements_CAA_pg5_', cd_state))
...
#clean up data
CWAinspections <- get(paste0('inspections_CWA_pg6_', cd_state))
...
#clean up data
CWAenforcement <- get(paste0('enforcements_CWA_pg6_', cd_state))
...
#clean up data
RCRAviolations <- get(paste0('violations_RCRA_pg7_', cd_state))
...
#clean up data
RCRAinspections <- get(paste0('inspections_RCRA_pg7_', cd_state))
...
#clean up data
RCRAenforcement <- get(paste0('enforcements_RCRA_pg7_', cd_state))
...
[Link to download PDF version of this report](https://github.com/edgi-govdata-archiving/CD-report/blob/master/reportcards/TX_2020.pdf)
